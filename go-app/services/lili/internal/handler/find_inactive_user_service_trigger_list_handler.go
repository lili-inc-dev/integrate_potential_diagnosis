...
func FindInactiveUserServiceTriggerListHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if _, err := middleware.GetFirebaseTokenFromReq(svcCtx.FirebaseAuth, r); err != nil {
			err = errorx.New(err, errorx.Unauthorized)
			httpx.Error(w, errors.Wrap(err, "FindInactiveUserServiceTriggerListHandler error"))
			return
		}

		l := logic.NewFindInactiveUserServiceTriggerListLogic(r.Context(), svcCtx)
		resp, err := l.FindInactiveUserServiceTriggerList()
		if err != nil {
			httpx.Error(w, errors.Wrap(err, "FindInactiveUserServiceTriggerListHandler error"))
		} else {
			httpx.OkJson(w, resp)
